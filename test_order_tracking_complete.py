...
async def test_complete_order_lifecycle():
    ...
    try:
        ...
        def on_order_closed(order_result):
            ...
            status = "WIN" if order_result.profit > 0 else "LOSE" if order_result.profit < 0 else "EVEN"
            print(f"🎯 ORDER COMPLETED via EVENT: {status} - Profit: ${order_result.profit:.2f}")
        ...
        print(f"⏱️ Waiting for trade to complete (up to {order_result.duration + 30} seconds)...")
        start_time = datetime.now()
        max_wait = timedelta(seconds=order_result.duration + 30)  # Trade duration + buffer
        ...
        while datetime.now() - start_time < max_wait:
            ...
            if result:
                ...
                if result.status != last_status:
                    status_emoji = "🟢" if result.status == "active" else "🔴" if result.status in ["win", "lose"] else "🟡"
                    print(f"   {status_emoji} Order status: {result.status}")
                    last_status = result.status
                
                # Check if order completed
                if result.profit is not None:
                    win_lose = "WIN" if result.profit > 0 else "LOSE" if result.profit < 0 else "EVEN"
                    print(f"\n🎯 TRADE COMPLETED!")
        ...
        if completed_orders:
            print(f"✅ Order completion detected via EVENT callback!")
            final_order_event = completed_orders[0]
            print(f"   Event Result - Profit: ${final_order_event.profit:.2f}")
        else:
            print(f"⚠️ No completion event received")
        
        # Check final polling result
        final_result_poll = await client.check_order_result(order_result.order_id)
        if final_result_poll:
            print(f"✅ Order completion detected via POLLING!")
            print(f"   Polling Result - Profit: ${final_result_poll.profit:.2f if final_result_poll.profit is not None else 'None'}")
        else:
            print(f"❌ Order not found via polling")
