    ...
    try:
        ...
        async with AsyncPocketOptionClient(session_id, is_demo=True) as client:
            ...
            def on_order_closed(order_result):
                logger.info(f"Order {order_result.order_id} closed with profit: ${order_result.profit:.2f}")
    ...
    async with AsyncPocketOptionClient(session_id, is_demo=True) as client:
        ...
        for asset in assets:
            try:
                ...
                direction = OrderDirection.CALL if len(orders) % 2 == 0 else OrderDirection.PUT
                ...
                logger.info(f"Placed {direction.value} order for {asset}: {order_result.order_id}")
        ...
        for order in orders:
            ...
            if result and result.profit is not None:
                ...
                logger.info(f"Order {order.order_id}: {result.status} - Profit: ${result.profit:.2f}")
                ...
                if len(price_history) >= 5:
                    ...
                    if abs(price_change) > 0.01:  # 0.01% threshold
                        ...
                        if balance.balance > 5:
                            direction = OrderDirection.CALL if price_change > 0 else OrderDirection.PUT
                            ...
                            try:
                                ...
                                logger.info(f"Strategy triggered: {direction.value} order placed: {order.order_id}")
