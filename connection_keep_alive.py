...
from pocketoptionapi_async.constants import REGIONS, CONNECTION_SETTINGS
from pocketoptionapi_async.exceptions import ConnectionError, WebSocketError
...
class ConnectionKeepAlive:
    ...
    def __init__(self, ssid: str, is_demo: bool = True):
        ...
        self.available_urls = REGIONS.get_demo_regions() if is_demo else REGIONS.get_all()
        ...
        logger.info(f"Initialized keep-alive manager with {len(self.available_urls)} available regions")
    ...
    async def stop_persistent_connection(self):
        ...
        tasks = [self._ping_task, self._reconnect_task, self._message_task, self._health_task]
        for task in tasks:
    ...
    async def _establish_connection(self) -> bool:
        ...
        for attempt in range(len(self.available_urls)):
            ...
            try:
                logger.info(f"🔌 Attempting connection to {url} (attempt {attempt + 1})")
                ...
                # Try next URL
                self.current_url_index = (self.current_url_index + 1) % len(self.available_urls)
        ...
        while self.should_reconnect:
            try:
                ...
                if self.connection_stats['last_ping_time']:
                    time_since_ping = datetime.now() - self.connection_stats['last_ping_time']
                    if time_since_ping > timedelta(seconds=60):  # No response for 60 seconds
                        logger.warning("🏥 Health check: No ping response, connection may be dead")
                        self.is_connected = False
            ...
            try:
                ...
                if not self.is_connected and self.should_reconnect:
                    ...
                    if self.current_reconnect_attempts <= self.max_reconnect_attempts:
                        logger.info(f"🔄 Reconnection attempt {self.current_reconnect_attempts}/{self.max_reconnect_attempts}")
                        ...
                        if success:
                            ...
                            logger.error(f"❌ Reconnection attempt {self.current_reconnect_attempts} failed")
...
async def demo_keep_alive():
    ...
    try:
        ...
        if success:
            logger.info("🚀 Keep-alive connection started, will maintain connection automatically...")
