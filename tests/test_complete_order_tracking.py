...
async def wait_for_trade_completion():
    ...
    try:
        ...
        if immediate_result:
            print(f"✅ Order immediately found in tracking system")
        else:
        ...
        max_wait = timedelta(seconds=order_result.duration + 30)  # Trade duration + 30 seconds buffer
        ...
        if final_result:
            ...
            if final_result.profit is not None:
                ...
                print(f"   ⚠️ No profit data available (may indicate tracking issue)")
        else:
