...
class IntegrationTester:
    ...
    async def run_full_integration_tests(self) -> Dict[str, Any]:
        ...
        for phase_name, phase_func in test_phases:
            ...
            try:
                ...
                logger.info(f"{status_emoji} {phase_name}: {'PASSED' if result['success'] else 'FAILED'} ({duration:.2f}s)")
    ...
    async def test_basic_connectivity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                results['regular_client'] = {'connected': True, 'balance_retrieved': balance is not None}
                await client.disconnect()
            else:
                results['regular_client'] = {'connected': False, 'balance_retrieved': False}
            
            # Test persistent client
            logger.info("Testing persistent connection client...")
            persistent_client = AsyncPocketOptionClient(self.ssid, is_demo=True, persistent_connection=True)
            success = await persistent_client.connect()
            if success:
                balance = await persistent_client.get_balance()
                results['persistent_client'] = {'connected': True, 'balance_retrieved': balance is not None}
                await persistent_client.disconnect()
            else:
                results['persistent_client'] = {'connected': False, 'balance_retrieved': False}
            ...
            if success:
                ...
                results['keep_alive'] = {'connected': True, 'message_sent': message_sent}
                ...
                try:
                    ...
                    if success:
                        ...
                        results[f'format_{i+1}'] = {
                            ...
                            'format': ssid_format[:50] + "..." if len(ssid_format) > 50 else ssid_format
                        }
                        await client.disconnect()
                    else:
                        results[f'format_{i+1}'] = {
    ...
    async def test_persistent_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            client = AsyncPocketOptionClient(self.ssid, is_demo=True, persistent_connection=True, auto_reconnect=True)
            
            # Connect
            success = await client.connect(persistent=True)
            if not success:
                return {'success': False, 'error': 'Failed to establish persistent connection'}
    ...
    async def test_keep_alive_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async def track_events(event_type):
                def handler(data):
                    events_received.append({'type': event_type, 'time': datetime.now(), 'data': data})
                return handler
            
            # Add event handlers
            keep_alive.add_event_handler('connected', await track_events('connected'))
            keep_alive.add_event_handler('message_received', await track_events('message'))
            keep_alive.add_event_handler('reconnected', await track_events('reconnected'))
            
            # Start connection
            success = await keep_alive.start_persistent_connection()
            if not success:
                return {'success': False, 'error': 'Failed to start keep-alive connection'}
            ...
            return {
                'success': len(connected_events) > 0 and messages_sent > 8,  # Most messages should succeed
                'details': {
    ...
    async def test_multi_client_scenarios(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if operation_tasks:
                ...
                total_operations = sum(r for r in operation_results if isinstance(r, int))
            else:
                total_operations = 0
            
            # Cleanup
            disconnect_tasks = [client.disconnect() for client in clients if client.is_connected]
            if disconnect_tasks:
                await asyncio.gather(*disconnect_tasks, return_exceptions=True)
            
            return {
                'success': successful_connections >= 2 and total_operations > 10,  # At least 2 clients, 10+ operations
                'details': {
                    'total_clients': len(clients),
                    'successful_connections': successful_connections,
                    'total_operations': total_operations,
                    'avg_operations_per_client': total_operations / max(successful_connections, 1)
                },
    ...
    async def test_error_recovery(self) -> Dict[str, Any]:
        ...
        try:
            ...
            client = AsyncPocketOptionClient(self.ssid, is_demo=True, auto_reconnect=True)
            ...
            errors_handled = sum(1 for scenario in error_scenarios if scenario['handled'])
    ...
    async def test_performance_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'success': good_throughput and good_success_rate and reasonable_duration,
                'details': {
    ...
    async def test_data_consistency_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Persistent client
            client2 = AsyncPocketOptionClient(self.ssid, is_demo=True, persistent_connection=True)
            ...
            if 'regular' in data_sources and 'persistent' in data_sources:
                ...
                balance_diff = abs(data_sources['regular']['balance'] - data_sources['persistent']['balance']) if data_sources['regular']['balance'] and data_sources['persistent']['balance'] else 0
                ...
                consistency_checks.append({
                    ...
                    'consistent': data_sources['regular']['candles_count'] == data_sources['persistent']['candles_count']
                })
            
            consistent_checks = sum(1 for check in consistency_checks if check['consistent'])
            
            return {
                'success': consistent_checks >= len(consistency_checks) * 0.8,  # 80% consistency
                'details': {
    ...
    async def test_long_running_stability(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success = await client.connect()
            if not success:
                return {'success': False, 'error': 'Failed to connect for stability test'}
            ...
            for i in range(1, len(operations_log)):
                gap = (operations_log[i]['time'] - operations_log[i-1]['time']).total_seconds()
                if gap > 10:  # More than 10 seconds gap
                    time_gaps.append(gap)
            
            return {
                'success': success_rate > 0.9 and len(time_gaps) == 0,  # 90% success rate, no major gaps
                'details': {
                    'total_operations': total_operations,
    ...
    def _generate_integration_report(self) -> Dict[str, Any]:
        ...
        test_duration = sum(result.get('duration', 0) for result in self.test_results.values())
        ...
        if health_score < 80:
            recommendations.append("System health below 80%. Review failed tests and address issues.")
        
        if failed_tests > 0:
            failed_test_names = [name for name, result in self.test_results.items() if result['status'] == 'FAILED']
            recommendations.append(f"Failed tests need attention: {', '.join(failed_test_names)}")
        
        if error_tests > 0:
            recommendations.append("Some tests encountered errors. Check logs for details.")
        
        if health_score >= 90:
            recommendations.append("Excellent system health! All major components working well.")
    ...
    def _assess_connectivity(self) -> Dict[str, Any]:
        ...
        connectivity_tests = ['Basic Connectivity', 'SSID Format Compatibility', 'Persistent Connection Integration']
        passed = sum(1 for test in connectivity_tests if self.test_results.get(test, {}).get('status') == 'PASSED')
        
        return {
            'score': (passed / len(connectivity_tests)) * 100,
            'status': 'GOOD' if passed >= len(connectivity_tests) * 0.8 else 'NEEDS_ATTENTION',
    ...
    def _assess_performance(self) -> Dict[str, Any]:
        ...
        performance_tests = ['Performance Under Load', 'Long-Running Stability', 'Multi-Client Scenarios']
        passed = sum(1 for test in performance_tests if self.test_results.get(test, {}).get('status') == 'PASSED')
        
        return {
            'score': (passed / len(performance_tests)) * 100,
            'status': 'GOOD' if passed >= len(performance_tests) * 0.8 else 'NEEDS_ATTENTION',
    ...
    def _assess_reliability(self) -> Dict[str, Any]:
        ...
        reliability_tests = ['Error Recovery', 'Keep-Alive Functionality', 'Data Consistency']
        passed = sum(1 for test in reliability_tests if self.test_results.get(test, {}).get('status') == 'PASSED')
        
        return {
            'score': (passed / len(reliability_tests)) * 100,
            'status': 'GOOD' if passed >= len(reliability_tests) * 0.8 else 'NEEDS_ATTENTION',
    ...
    def _assess_monitoring(self) -> Dict[str, Any]:
        ...
        passed = sum(1 for test in monitoring_tests if self.test_results.get(test, {}).get('status') == 'PASSED')
        
        return {
            'score': (passed / len(monitoring_tests)) * 100 if len(monitoring_tests) > 0 else 100,
            'status': 'GOOD' if passed >= len(monitoring_tests) * 0.8 else 'NEEDS_ATTENTION',
...
async def run_integration_tests(ssid: str = None):
    ...
    try:
        ...
        logger.info(f"Health Score: {summary['health_score']:.1f}/100 ({summary['health_status']})")
        ...
        for aspect, details in assessment.items():
            ...
            logger.info(f"{status_emoji} {aspect.title()}: {details['score']:.0f}/100 - {details['details']}")
        ...
        if summary['health_score'] >= 90:
            ...
            logger.info("👍 GOOD: System is performing well with minor areas for improvement")
        elif summary['health_score'] >= 60:
            logger.warning("⚠️ FAIR: System has some issues that should be addressed")
        else:
            logger.error("❌ POOR: System has significant issues requiring immediate attention")
