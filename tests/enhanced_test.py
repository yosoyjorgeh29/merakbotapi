...
from pocketoptionapi_async import (
    AsyncPocketOptionClient, OrderDirection, error_monitor, health_checker,
...
class EnhancedAPITester:
    ...
    async def handle_error_alert(self, alert_data):
        ...
        logger.warning(f"🚨 ERROR ALERT: {alert_data['error_type']} - "
    ...
    async def test_enhanced_connection(self):
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"📊 Performance: {len(metrics['operation_metrics'])} tracked operations")
    ...
    async def test_monitored_operations(self, client):
        ...
        for op_name, operation in operations:
            try:
                ...
                result = await client.execute_with_monitoring(
...
async def main():
    ...
    if session_id == 'test_session_id':
        logger.warning("⚠️  Using test session ID - set POCKET_OPTION_SSID for real testing")
