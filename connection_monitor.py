...
class ConnectionMonitor:
    ...
    async def start_monitoring(self, persistent_connection: bool = True) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.success(f"✅ Monitoring started (connection time: {connection_time:.3f}s)")
                return True
            else:
    ...
    def get_real_time_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'error_rate': self.total_errors / max(self.total_messages, 1),
            'messages_per_second': self.total_messages / uptime.total_seconds() if uptime.total_seconds() > 0 else 0,
    ...
    def get_historical_metrics(self, hours: int = 1) -> Dict[str, Any]:
        ...
        # Calculate trends
        if recent_snapshots:
            memory_values = [s.memory_usage_mb for s in recent_snapshots if s.memory_usage_mb > 0]
            response_values = [s.avg_response_time for s in recent_snapshots if s.avg_response_time > 0]
            ...
            if response_values:
                historical['response_time_trend'] = {
                    ...
                    'trend': 'improving' if len(response_values) > 1 and response_values[-1] < response_values[0] else 'stable'
                }
    ...
    def generate_diagnostics_report(self) -> Dict[str, Any]:
        ...
        if stats.get('avg_response_time', 0) > 3.0:
            health_score -= 15
            health_issues.append(f"Slow response time: {stats.get('avg_response_time', 0):.2f}s")
        
        if stats['connection_success_rate'] < 0.9:
            health_score -= 10
            health_issues.append(f"Low connection success rate: {stats['connection_success_rate']:.1%}")
        ...
        if stats['error_rate'] > 0.1:
            recommendations.append("High error rate detected. Check network connectivity and SSID validity.")
        
        if stats.get('avg_response_time', 0) > 5.0:
            recommendations.append("Slow response times. Consider using persistent connections or different region.")
        ...
        report = {
            ...
            'connection_summary': {
                ...
                'current_status': 'CONNECTED' if stats['is_connected'] else 'DISCONNECTED',
