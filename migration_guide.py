...
class PocketOptionMigrationWrapper:
    ...
    def __init__(self, ssid: str, demo: bool = True):
        ...
        self.ssid = ssid
        self.demo = demo
        self.client = AsyncPocketOptionClient(session_id=ssid, is_demo=demo)
    ...
    async def buy(
        self, amount: float, active: str, action: str, expirations: int
        ...
        try:
            ...
            direction = (
                OrderDirection.CALL if action.lower() == "call" else OrderDirection.PUT
            )

            # Place order
            result = await self.client.place_order(
                asset=active, amount=amount, direction=direction, duration=expirations
            )
    ...
    async def get_candles(self, active: str, period: int, count: int = 100) -> list:
        ...
        try:
            candles = await self.client.get_candles(
                asset=active, timeframe=period, count=count
            )
...
async def migration_example_old_style():
    ...
    async with PocketOptionMigrationWrapper(ssid, demo=True) as api:
        ...
        success, order_id = await api.buy(
            amount=1.0, active="EURUSD_otc", action="call", expirations=60
        )
...
async def migration_example_new_style():
    ...
    async with AsyncPocketOptionClient(ssid, is_demo=True) as client:
        ...
        def on_order_closed(order_result):
            ...
            print(
                f"📊 Order {order_result.order_id}: {status} - Profit: ${order_result.profit:.2f}"
            )
        ...
        try:
            ...
            df = await client.get_candles_dataframe(
                asset="EURUSD_otc", timeframe="1m", count=50
            )
