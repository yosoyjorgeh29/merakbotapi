...
async def test_ssid_formats():
    ...
    # Compare key fields
    fields_match = (
        auth_data1["session"] == auth_data2["session"] and
        auth_data1["isDemo"] == auth_data2["isDemo"] and
        auth_data1["uid"] == auth_data2["uid"] and
        auth_data1["platform"] == auth_data2["platform"]
    )
    ...
    print('    ssid=\'42["auth",{"session":"your_session","isDemo":1,"uid":12345,"platform":1,"isFastHistory":true}]\'')
    print(")")
...
async def test_real_connection_simulation():
    ...
    print(f"Initialized client with parsed data:")
