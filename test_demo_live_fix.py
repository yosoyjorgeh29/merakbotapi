...
async def test_demo_live_fix():
    ...
    print("\n1️⃣ Test: is_demo=True with demo SSID")
    ...
    print(f"   Constructor is_demo: True")
    ...
    print("\n2️⃣ Test: is_demo=False with demo SSID")
    ...
    print(f"   SSID isDemo value: {json.loads(demo_ssid[10:-1])['isDemo']}")
    print(f"   Constructor is_demo: False")
    ...
    print("\n3️⃣ Test: Raw session with is_demo=True")
    raw_session = "n1p5ah5u8t9438rbunpgrq0hlq"
    client_raw_demo = AsyncPocketOptionClient(ssid=raw_session, is_demo=True, uid=72645361)
    ...
    print("\n4️⃣ Test: Raw session with is_demo=False")
    client_raw_live = AsyncPocketOptionClient(ssid=raw_session, is_demo=False, uid=72645361)
    ...
    demo_region_names = [name for name, url in all_regions.items() if url in demo_regions]
    print(f"   Should use demo regions: {demo_region_names}")
    
    # Check live client region selection  
    print(f"\n   Live client (is_demo=False):")
    live_region_names = [name for name, url in all_regions.items() if "DEMO" not in name.upper()]
