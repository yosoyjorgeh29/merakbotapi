...
class PerformanceTester:
    ...
    async def test_connection_performance(self, iterations: int = 5) -> Dict[str, float]:
    ...
    async def test_order_placement_performance(self, iterations: int = 10) -> Dict[str, float]:
        ...
        if order_times:
            return {
                ...
                'orders_per_second': 1 / statistics.mean(order_times) if order_times else 0
            }
        else:
            return {'success_rate': 0}
    ...
    async def test_concurrent_operations(self, concurrency_level: int = 5) -> Dict[str, Any]:
    ...
    async def generate_performance_report(self) -> str:
        ...
        try:
            ...
            if conn_results.get('success_rate', 0) > 0:
                report.append(f"✅ Average Connection Time: {conn_results['avg_time']:.3f}s")
                report.append(f"✅ Min Connection Time: {conn_results['min_time']:.3f}s")
                report.append(f"✅ Max Connection Time: {conn_results['max_time']:.3f}s")
        ...
        try:
            ...
            for operation, stats in data_results.items():
                ...
                report.append(f"    Range: {stats['min_time']:.3f}s - {stats['max_time']:.3f}s")
        except Exception as e:
        ...
        try:
            ...
            if concurrent_results.get('success_rate', 0) > 0:
                report.append(f"✅ Success Rate: {concurrent_results['success_rate']:.1f}%")
                report.append(f"✅ Operations/Second: {concurrent_results['operations_per_second']:.2f}")
                report.append(f"✅ Avg Operation Time: {concurrent_results['avg_operation_time']:.3f}s")
